...
def profile():
    ...
    if form.validate_on_submit():
        password = form.password.data
        correct_password = User.authenticate(g.user.username, password)
        if not correct_password:
            flash(f"Incorrect password for {g.user.username}.", "danger")
            return redirect(url_for("homepage"))
...
def messages_destroy(message_id):
    ...
    if msg.user_id != g.user.id:
        flash("You cannot delete other's messages.", "danger")
        return redirect("/")
...
def homepage():
    ...
    if g.user:
        ...
        messages = (Message
                    .query
                    .filter(Message.user_id.in_([g.user.id, *following_ids]))
                    .order_by(Message.timestamp.desc())
                    .limit(100)
                    .all())
